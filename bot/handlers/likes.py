...
async def on_like(callback: CallbackQuery) -> None:
    ...
    async with BackendClient() as client:
        try:
            data = await client.toggle_like(user_id, track_id)
...
async def on_dislike(callback: CallbackQuery) -> None:
    ...
    async with BackendClient() as client:
        try:
            data = await client.toggle_dislike(user_id, track_id)
            disliked: bool = data.get("disliked", False)
            logger.info("dislike_toggled", disliked=disliked)
            text = "👎 Дизлайк поставлен" if disliked else "👌 Дизлайк убран"
            await callback.answer(text, show_alert=False)
        except BackendError as exc:
            logger.error(
